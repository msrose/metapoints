  ...
  return {
    ...
    all: function() {
      var list = [
        ...
        {
          title: "Unlock Decorations",
          socketEvent: "unlock decorations",
          socketHandler: "unlockDecorations",
          description: "Spend " + config.decorationsCost + " Metapoints"
        }
      ];
      return list;
    },
    ...
    doubleOrNothing: function(person, data, callback) {
      ...
      var probability = config.doubleOrNothingChanceFactor + Math.min(person.luck * config.luckIncreaseFactor, config.luckMaxIncrease);
      if(seed < probability) {
        person.metapoints *= 2;
        callback(null, "Congratulations, you just doubled your metapoints!", "Probability: " + probability + ", Seed: " + seed);
      } else {
        person.metapoints = 0;
        callback(null, "You lost all of your metapoints!", "Probability: " + probability + ", Seed: " + seed);
      }
    },
    ...
    unlockDecorations: function(person, data, callback) {
      if(person.decorations) {
        return callback(null, "Already unlocked!");
      }
      if(person.metapoints >= config.decorationsCost) {
        person.decorations = {};
        person.metapoints -= config.decorationsCost;
        callback(null, "Unlocked decorations!");
      } else {
